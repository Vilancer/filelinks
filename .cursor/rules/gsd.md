...
**Owner:** Malek Elkssas  
**Project:** filelinks (maintainer / primary driver)  
**Updated:** 2026-06-05

### Role & goals

- Ships **filelinks** via GSD milestones (v1.0 MVP, v1.1 AI agents complete).
- Next focus: **define v1.2** from post–v1.1 backlog (git hooks, graph, suggest, editor/Nx tooling, more providers).
- Values **demoable CLI + core** first; defers hooks/editor until core/agent path is solid.

### Stack & workflow

- **Stack:** TypeScript, Nx, pnpm, Vitest, Effect Schema, Commander, Ink (CLI `add`).
- **Agents:** Cursor (`check --run-agents`), GSD commands (`/gsd-quick`, `/gsd-execute-phase`, `/gsd-new-milestone`).
- **GSD config** (`.planning/config.json`): `model_profile: balanced`, `mode: yolo`, `granularity: coarse`, research + verifier on, `commit_docs: true`.
- **Branching:** GSD templates (`gsd/phase-*`, `gsd/{milestone}-*`); cloud agent branches use `cursor/*-e65b`.
